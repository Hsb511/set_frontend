...
class GameStateMachineTest {
    ...
    fun `Given a 15 to 12 cards table configuration, When selecting the set, Then the table has 12 cards correctly compacted`() =
        runTest {
            ...
            val card1 = createCard(color = Color.PRIMARY, shape = Shape.OVAL, number = 1, fill = Fill.SOLID)
            val card2 = createCard(color = Color.PRIMARY, shape = Shape.OVAL, number = 2, fill = Fill.STRIPED)
            val card3 = createCard(color = Color.TERTIARY, shape = Shape.OVAL, number = 1, fill = Fill.SOLID)
    ...
    fun `Init event on Finished or Playing state does nothing`() = runTest {
        // Given
        val playing = createPlayingGame(deck = emptyList(), table = emptyList())
        ...
        return colors.flatMap { color ->
            shapes.flatMap { shape ->
                numbers.flatMap { number ->
                    fills.map { fill ->
                        createCard( number, color, shape,  fill)
                    }
                }
            }
        }
